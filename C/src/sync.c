...
void shaSyncInput(ShaLink *link, ShaGuest *guest, ShaPack *chunk) {
    ...
    if (sync->youTime != 0) {
        ...
        rtt = (rtt > (MCS)sync->mePause) ? rtt - sync->mePause : 0;
    }
    MCS rtts = 0;
    ...
    terminal->statistic.syncsIn++;
    terminal->statistic.meRTT = (guest != NULL) ? guest->brief.rttAvg : link->brief.rttAvg;
    terminal->statistic.youRTT = sync->meRTT;
    ShaSyncPair *pairs = (ShaSyncPair*)((uint8_t*)sync + sizeof(ShaSync));
    for (uint32_t count = 0; count < sync->countHoles; count++) {
...
void shaSyncOutput(ShaLink *link, ShaGuest *guest) {
    ShaTerminal *terminal = link->terminal;
    ShaPack *chunk = shaChunkBuildSync();
    ShaSync *sync = (ShaSync*)chunk->data;
    ShaBrief *brief = (guest != NULL) ? &guest->brief : &link->brief;
    MCS now = GetNow();
    sync->meTime = now;
    sync->mePause = (brief->meTime != 0) ? now - brief->meTime : 0;
    sync->youTime = brief->youTime;
