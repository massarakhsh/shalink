...
void linkInputChunk(ShaLink *link, ShaGuest *guest, ShaPack *chunk) {
    ShaTerminal *terminal = link->terminal;
    terminal->statistic.pksTotalIn++;
    terminal->statistic.btsTotalIn += PackHeadSize + chunk->head.sizeData;
...
void linkInputCode(ShaLink *link, ShaGuest *guest, const uint8_t *code, uint16_t size) {
    ...
    const PackHead *head = (const PackHead*)code;
    uint16_t sizeData = head->sizeData;
    uint16_t sizeCode = PackHeadSize+sizeData;
    if (sizeCode > size) {
        printf("ERROR inputCode size=%d sizeCode=%d\n", (int)size, (int)sizeCode);
        return;
    }
    ShaPack *chunk = shaChunkBuildCode(head, sizeCode);
    if (chunk == NULL) {
